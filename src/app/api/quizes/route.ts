  ...
  try {
    ...
    if (videoFolderId) {
      try {
        ...
        console.log(`Found ${driveVideos.length} video files in Google Drive for quizes`);
      } catch (error) {
        console.warn('Could not fetch video files from Google Drive for quizes:', error);
        // Continue with local files only
      }
    } else {
      console.log('Google Drive video folder not configured, using local files only for quizes');
    }
    ...
    // Combine local metadata with Google Drive URLs for quiz purposes
    const quizVideos: any = {};
    ...
    if (localVideoData.videos) {
      Object.entries(localVideoData.videos).forEach(([videoId, metadata]: [string, any]) => {
        ...
        quizVideos[videoId] = {
          ...
          quiz_available: !!driveVideo // Only show videos that are available in Google Drive
        };
      });
    }
    ...
    driveVideos.forEach(driveVideo => {
      ...
      if (!quizVideos[nameWithoutExt]) {
        quizVideos[nameWithoutExt] = {
    ...
    // Filter to only return videos that have Google Drive files (for quiz functionality)
    const availableQuizVideos = Object.fromEntries(
      Object.entries(quizVideos).filter(([_, video]: [string, any]) => video.quiz_available)
    );
    
    return NextResponse.json({ 
      videos: availableQuizVideos,
      total_count: Object.keys(availableQuizVideos).length
    });
  } catch (error) {
    console.error('Error fetching quiz videos:', error);
    return NextResponse.json(
      { error: 'Failed to fetch quiz videos' },
      { status: 500 }
    );
  }
}